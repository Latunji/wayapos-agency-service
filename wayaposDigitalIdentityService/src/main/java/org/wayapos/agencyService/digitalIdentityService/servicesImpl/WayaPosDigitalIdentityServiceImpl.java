...
public class WayaPosDigitalIdentityServiceImpl implements WayaPosDigitalIdentityServices {
	...
	public Response superAdminAccountCreation(SuperAdminAccountCreationReq superAdminAccountCreationReq) {
		...
		Gson gson = new Gson();
		String gsonString = gson.toJson(superAdminAccountCreationReq);
		System.out.println(gsonString);

		String url = "https://services.staging.wayabank.ng/auth-service/api/v1/auth/create";

		HttpHeaders headers = new HttpHeaders();
		headers.setAccept(Arrays.asList(MediaType.APPLICATION_JSON));
		HttpEntity<SuperAdminAccountCreationReq> entity = new HttpEntity<SuperAdminAccountCreationReq>(superAdminAccountCreationReq, headers);
