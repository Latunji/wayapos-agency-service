...
public class WayaPosDigitalIdentityController {
	...
	@PostMapping("/superAdmin/AccountCreation")
	public Response superAdminAccountCreation(@RequestBody SuperAdminAccountCreationReq superAdminAccountCreationReq) {
		return wayaPosDigitalIdentityService.superAdminAccountCreation(superAdminAccountCreationReq);
	}
	...
	public Response adminLogin(@RequestBody LoginRequest loginRequest) {
		Response loginResponse = wayaPosDigitalIdentityService.adminLogin(loginRequest);
		
		if(!loginResponse.getData().getUser().getIsAdmin()) {
			loginResponse.setData(null);
			loginResponse.setMessage("User is not an admin");
